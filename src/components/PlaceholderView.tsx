  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-4xl mx-auto py-12">
      <GlassCard className="flex flex-col items-center justify-center py-24 text-center gap-8 relative overflow-hidden group">
        ...
        <div className="w-28 h-28 bg-[#1a1a1a] rounded-3xl flex items-center justify-center shadow-lg border border-[#333] group-hover:scale-110 transition-transform duration-500">
          {icon}
        </div>
        <div className="z-10">
          <h3 className="text-4xl font-black text-white tracking-tighter uppercase mb-4">{title}</h3>
          <p className="text-slate-500 font-medium max-w-sm mx-auto leading-relaxed">{desc}</p>
        </div>
        <div className="mt-4 px-8 py-3 bg-violet-600/10 rounded-2xl border border-violet-500/20 shadow-lg">
