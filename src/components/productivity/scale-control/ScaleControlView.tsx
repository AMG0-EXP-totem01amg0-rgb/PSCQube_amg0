...
import { Activity, Plus, Trash2, Save, Scale, Calendar, FilterX, RefreshCcw } from 'lucide-react';
...
export default function ScaleControlView({ masters, currentUser, onSave, onDelete, history, selectedShiftId, selectedDate }: Props) {
  ...
  useEffect(() => {
    if (dateFrom && dateTo) {
      ...
      fetchTableFromSheets("CONTROL_BALANZAV2", true, { dateFrom, dateTo }, "ScaleControlView.range")
        .then(result => {
